...
do_uninstall() {
    ...
    # 4. Symlink Removal
    local links=(~/.vimrc ~/.vim ~/.tmux.conf ~/.inputrc ~/.dir_colors ~/.bin ~/.config/nvim ~/.local/share/fonts/dotfiles)
    for l in "${links[@]}"; do
        if [[ -L "$l" ]]; then
            run rm "$l"
            success "Removed $l"
...
do_status() {
    ...
    local components=(
        ...
        "Vim Dir       :~/.vim:[ -L ~/.vim ]"
        "Neovim        :~/.config/nvim:[ -L ~/.config/nvim ]"
        "Utilities     :~/.bin:[ -L ~/.bin ]"
