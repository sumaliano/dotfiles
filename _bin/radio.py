...
def probePlayer():
    ...
    #implementedPlayers = Player.__subclasses__() #the original
    #implementedPlayers = [MpPlayer, VlcPlayer] 
    implementedPlayers = [VlcPlayer] # MpPlayer does not respond to volume commands and mute

    if logger.isEnabledFor(logging.DEBUG):
        logger.debug("implementedPlayers: {} ".format(str(implementedPlayers)))
...
class PyRadio(object):
    ...
    def setup(self, stdscr):
        ...
        self.player = probePlayer()(self.log)
...
DEFAULT_FILE = './stations.csv'
for p in [os.path.join(os.getenv('HOME', '~'), '.pyradio', 'stations.csv'),
          os.path.join(os.getenv('HOME', '~'), '.pyradio'),
          os.path.join(os.path.dirname(__file__), 'stations.csv')]:
...
def shell():
    ...
    if args.add:
        params = eval(input("Enter the name: ")), eval(input("Enter the url: "))
